...
let () =
  ...
  let filtered_df =
    Df.filter
      df
      Df.R.(
        let+ pi = (column N.intarr) col_pi in
        pi = 1)
  in
  ...
  let sum_df =
    Df.map
      ...
      N.floatarr
      Df.R.(
        let+ pi = (column N.intarr) col_pi
        and+ e = (column N.floatarr) col_e2 in
        Float.of_int pi +. e)
  in
  Stdio.printf "> %d\n%!" (Df.length df);
  ...
  let sum_df =
    Df.map
      ...
      N.floatarr
      Df.R.(
        map2 (column N.intarr col_pi) (column N.floatarr col_e2) ~f:(fun pi e ->
            Float.of_int pi +. e))
  in
  Stdio.printf "> %d\n%!" (Df.length df);
  Stdio.print_endline (Column.to_string sum_df);
  let sorted_df = Df.sort df Df.R.((column N.intarr) col_pi) ~compare:Int.compare in
